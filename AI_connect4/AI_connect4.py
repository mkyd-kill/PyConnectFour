# AI_connect4 is a game where the player plays with a simple AI as player two
# We are implementing the random drop circle for player to into a simple AI 
...
""" 
	terminal node is the point at which the user or player wins, 
	or we have checked all the pieces of the board
"""
...
def minimax(board, depth, alpha, beta, maximazingPlayer):
	...
	if maximazingPlayer: # for the maximizing player that is the AI
		value = -math.inf
		...
		for col in valid_location:
			...
			if new_score > value:
				value = new_score
				column = col
				alpha = max(alpha, value) # using the alpha beta pruning
				if alpha >= beta:
					break
		return column, value

	else: # for the minimizing player that is the user
		value = math.inf
		column = random.choice(valid_location)
		for col in valid_location:
			row = get_next_open_row(board, col)
			...
			if new_score < value:
				value = new_score
				column = col
				beta = min(beta, value) # using the alpha beta pruning
				if alpha >= beta:
					break
		return column, value
...
def pick_best_move(board, piece):
	...
	for col in valid_location:
		...
		# this modifies the score of the board
